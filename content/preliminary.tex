...
\paragraph{} A basic functional implementation of the simulation has been completed, viewable in Appendix \ref{appendix:code} \todo{is it necessary to put this in the appendix?} or available at \url{https://github.com/hans-m-song/harbergers-tax}. The simulation is an implementation of the pseudocode below, with Algorithm \ref{algorithm:trade} being called at intervals based on the pool parameters and Algorithm \ref{algorithm:reward} being called at intervals based on the blockchain parameters. An alternate reward system will also be simulated in which Algorithm \ref{algorithm:reward} will reward the pool directly and Algorithm \ref{algorithm:chunkreward} will be executed, distributing the reward to the players
